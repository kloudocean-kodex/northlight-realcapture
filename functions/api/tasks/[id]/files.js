...
async function approvedReleaseFiles(env, task) {
  if (!task.approved_release_id) return [];
  const release = (await supa(env, 'media_releases', {
    query: `select=id,status&id=eq.${encodeURIComponent(task.approved_release_id)}&task_id=eq.${encodeURIComponent(task.id)}&status=eq.approved&limit=1`,
  }))?.[0];
  if (!release) return [];
...
export async function onRequestGet({ request, env, params }) {
  ...
  try {
    ...
    const approved = stages.includes('03_FINAL') ? await approvedReleaseFiles(env, task) : [];
    const files = [...(indexed || []), ...approved].sort((a, b) =>
      String(a.stage).localeCompare(String(b.stage)) ||
      String(a.service_code || '').localeCompare(String(b.service_code || '')) ||
      String(a.name || '').localeCompare(String(b.name || '')),
    );
    return json({ files, stages });
  } catch {
