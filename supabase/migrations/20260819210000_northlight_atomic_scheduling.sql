  ...
  if not exists (
    ...
    alter table public.tasks
      ...
      exclude using gist (
        ...
        tstzrange(
          scheduled_start - make_interval(mins => public.northlight_schedule_buffer_minutes(metadata, 'buffer_before_min')),
          scheduled_end + make_interval(mins => public.northlight_schedule_buffer_minutes(metadata, 'buffer_after_min')),
          '[)'
        ) with &&
      )
      where (
