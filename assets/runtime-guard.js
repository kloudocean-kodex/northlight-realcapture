(()=>{
  ...
  let lastTask=null,bookingArea='',bookingServices=[];
  ...
  function captureBookingContext(){
    const area=document.querySelector('#wArea');
    if(area){bookingArea=area.value||bookingArea;if(!area.dataset.nlBookingContext){area.dataset.nlBookingContext='1';const remember=()=>{bookingArea=area.value||bookingArea};area.addEventListener('change',remember);area.addEventListener('input',remember)}}
    const services=[...document.querySelectorAll('[data-service]')];
    if(services.length)bookingServices=services.filter(x=>x.classList.contains('on')).map(x=>x.dataset.service).filter(Boolean);
  }
  function enrichFreebusy(init={}){
    try{const payload=JSON.parse(String(init.body||'{}'));if(!payload.area&&bookingArea)payload.area=bookingArea;if((!Array.isArray(payload.services)||!payload.services.length)&&bookingServices.length)payload.services=[...bookingServices];return{...init,body:JSON.stringify(payload)}}catch{return init}
  }
  ...
  function applySlotResult(d){setTimeout(()=>{const btn=document.querySelector('#nextStep'),box=document.querySelector('#slotCheck');if(btn)btn.disabled=d?.available!==true;if(box){box.innerHTML='';const icon=document.createElement('span');if(window.NLIcon)icon.innerHTML=window.NLIcon(d?.available?'check':'alert');else icon.textContent=d?.available?'✓':'△';const strong=document.createElement('strong');strong.textContent=d?.available?' Available.':' Not available.';const text=document.createElement('span');text.textContent=` ${d?.reason||'Choose another time.'}`;box.append(icon,strong,text)}},0)}
  ...
  window.fetch=async function(input,init={}){
    ...
    const freebusy=url.includes('/api/calendar/freebusy')&&method==='POST';
    let requestInit=init;
    if(freebusy){captureBookingContext();slotChecking();requestInit=enrichFreebusy(init)}
    const response=await nativeFetch(input,requestInit);
    if(freebusy){try{const d=await response.clone().json();applySlotResult(d)}catch{applySlotResult({available:false,reason:'Availability could not be verified.'})}}
  ...
  async function addRecipientControl(){
    const titleText=document.querySelector('.topbar h2')?.textContent?.trim()||'',roleText=document.querySelector('.role-pill')?.textContent||'',isAdmin=/^Admin\b/i.test(roleText),isOwner=/(Owner|Principal)/i.test(roleText),settingsGrid=document.querySelector('#page .settings-grid'),integrationGrid=document.querySelector('#page .integration-grid'),grid=isAdmin&&titleText==='Settings'?settingsGrid:isOwner&&titleText==='Integrations'?integrationGrid:null;
    if(!grid||document.querySelector('#operationsRecipientCard'))return;
    const card=document.createElement('div');card.id='operationsRecipientCard';card.className=isOwner?'card integration':'card section-card';card.innerHTML=`<span class="service-icon">${window.NLIcon?window.NLIcon('mail'):''}</span><h3>Notification recipient</h3><p class="detail-copy">Assignment mail goes to the assigned photographer when a real email is configured. This address is the operational fallback for pilot accounts and task alerts.</p><div class="field"><label for="operationsEmail">Operations email</label><input id="operationsEmail" type="email" autocomplete="email" placeholder="operations@realcapture.com.au"></div><button class="btn primary" id="saveOperationsEmail">Save recipient</button><div class="hint" id="operationsEmailStatus">Loading current recipient…</div>`;
    grid.appendChild(card);
    try{const r=await nativeFetch('/api/settings',{credentials:'include'}),d=await r.json();if(!r.ok)throw new Error(d.error||'Could not load recipient');const input=card.querySelector('#operationsEmail'),status=card.querySelector('#operationsEmailStatus');input.value=d.operationsEmail||'';status.textContent=d.source==='workspace'?'Managed in Northlight.':'Currently using the deployment fallback; save here to manage it in Northlight.';card.querySelector('#saveOperationsEmail').onclick=async()=>{const btn=card.querySelector('#saveOperationsEmail');btn.disabled=true;try{const rr=await nativeFetch('/api/settings',{method:'PATCH',credentials:'include',headers:jsonHeaders,body:JSON.stringify({operationsEmail:input.value.trim()})}),dd=await rr.json();if(!rr.ok)throw new Error(dd.error||'Could not save recipient');status.textContent='Saved in Northlight.';toast('Notification recipient updated',dd.operationsEmail||'Deployment fallback restored.')}catch(e){toast('Could not update recipient',e.message)}finally{btn.disabled=false}}}catch(e){card.querySelector('#operationsEmailStatus').textContent=e.message}
  }
